...
public class MainScreen extends BaseScreen {
    ...
    public void updateShot(Shot shot, float deltaTime){
		...
		if (this.player.velocity.x > 0) {
			startX = endX = (int)((this.player.desiredPosition.x + this.player.velocity.x + this.player.getWidth()) / TILED_SIZE);
		}
		else {
			startX = endX = (int)((this.player.desiredPosition.x + this.player.velocity.x) / TILED_SIZE);
		}

		if (this.player.grounded && normalGravity){
			startY = (int)((this.player.desiredPosition.y) / TILED_SIZE) + 1;
			endY = (int)((this.player.desiredPosition.y + this.player.getHeight()) / TILED_SIZE) + 1;
		}
		else if (this.player.grounded && !normalGravity){
			startY = (int)((this.player.desiredPosition.y) / TILED_SIZE) - 1;
			endY = (int)((this.player.desiredPosition.y + this.player.getHeight()) / TILED_SIZE) - 1;
		}
		else{
			startY = (int)((this.player.desiredPosition.y) / TILED_SIZE);
			endY = (int)((this.player.desiredPosition.y + this.player.getHeight()) / TILED_SIZE);
		}
		...
		if (normalGravity){
			if (this.player.velocity.y > 0) {
				startY = endY = (int)((this.player.desiredPosition.y + this.player.velocity.y + this.player.getHeight()) / TILED_SIZE);
			}
			else {
				startY = endY = (int)((this.player.desiredPosition.y + this.player.velocity.y) / TILED_SIZE);
			}
		}
		else{
			if (this.player.velocity.y < 0) {
		...
		startX = (int)(this.player.desiredPosition.x / TILED_SIZE);
		endX = (int)((this.player.desiredPosition.x + this.player.getWidth()) / TILED_SIZE);
	...
	private void updatePlayer(float deltaTime) {
		...
		for (Shot shot : this.shotArray){
			if (shot != null)
				this.updateShot(shot, deltaTime);		//pool of shots?
		}
		if (this.normalGravity)
